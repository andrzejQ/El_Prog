...
cmd002=cmd
param002=/k for %%G in ("%S") do ( ffmpeg.exe -i %%G -c:v libx264 -crf 22 -preset slow -map_metadata 0 %%G.mp4 )
menu003=ffmpeg: zaznaczenie --> x264 i zachowaj datę
cmd003=cmd
param003=/k for %%G in ("%S") do ( ffmpeg.exe -i %%G -c:v libx264 -crf 22 -preset slow -map_metadata 0 %%G.mp4 && py "%%FileDate%%\examples\copyFileDate.py" %%G %%G.mp4 )
````
