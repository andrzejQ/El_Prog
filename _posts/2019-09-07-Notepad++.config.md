...
* <https://github.com/andrzejQ/filedate> <small>(zmienną środowiskową `%FileDate%` należy ustawić na pełną ścieżkę do foldera `filedate\Files`)</small>
...
cmd002=cmd
param002=/k for %%G in ("%S") do py "%%FileDate%%\examples\copyFileDate.py" %T%M %%Gv
...
<details markdown=1><summary markdown="span">`ffmpeg` nieźle zmniejsza rozmiar JPG, ale jest problem z kopiowaniem metadanych...</summary>

▿  
Zmniejszanie plików JPG (z transferem metadanych EXIF i daty pliku):

Aby przenieść metadane do zmniejszonego pliku można użyć `ExifTool`.
...
````
Choć zdania na temat jakości konwersji JPG za pomocą FFMPEG są podzielone. 
...
Zob. też [Hurtowe zmniejszanie dużych obrazów *.jpg](https://andrzejq.github.io/Office_S_Tips/system/2019/09/20/drobne_podpowiedzi_2.html#hurtowe-zmniejszanie-du%C5%BCych-obraz%C3%B3w-jpg) z użyciem IrfanView.

Warto też wypróbować narzędzie <https://imagemagick.org/>, które przy okazji konwersji przenosi też metadane:

````bat
mkdir mniejsze
for %%F in (*.jpg) do (
  magick "%%F" -resize 50% "mniejsze/%%F"
  py "%%FileDate%%\examples\copyFileDate.py" "%%F" "mniejsze/%%F" )
)
````
lub PowerShell:
````powershell
New-Item -Path ".\mniejsze" -ItemType Directory *>$null
foreach ($F in Get-ChildItem -Filter *.jpg) {
  magick -verbose "$F" -resize 50% "mniejsze/$F";
  (Get-Item "mniejsze/$F").LastWriteTime = (Get-Item $F).LastWriteTime
}
````
...
````ini
menu001=-__md-it i mammoth-docx -> html
menu002=md-it.cmd -> html
cmd002=cmd
param002=/k md-it.cmd %N
menu003=html -> _1.html (img inline)
cmd003=cmd
param003=/k embed-img.cmd %N
menu004=docx -> html
cmd004=cmd
param004=/k py %%AppData%%/mmth/mammothH.py %N
menu005=docx -> html (+d.img)
cmd005=cmd
param005=/k py %%AppData%%/mmth/mammothH.py %N --img-dir d.img
menu006=--
````
